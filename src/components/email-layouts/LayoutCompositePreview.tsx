...
export function LayoutCompositePreview({
  ...
  if (previewItems.length === 0) {
    return (
      <div
        className={`relative grid min-h-[320px] place-items-center rounded-[1rem] border border-slate-200 bg-slate-50 text-[1rem] text-slate-600 ${className}`}
      >
  ...
  return (
    <div
      className={`relative overflow-hidden rounded-[1rem] border border-slate-200 bg-slate-50 shadow-[0_18px_36px_rgba(0,0,0,0.32)] ${className}`}
    >
      <div
      ...
      >
        {previewItems.map((item, index) => (
          <div
            ...
            className="relative min-h-[160px] border border-slate-200 bg-slate-50"
          >
            <Image
            ...
            />
            <div className="absolute left-2 top-2 inline-flex h-6 min-w-6 items-center justify-center rounded-[0.35rem] border border-slate-200 bg-white px-1.5 text-[0.72rem] font-bold text-slate-900">
              {index + 1}
            </div>
          </div>
        ))}
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/55 to-transparent px-4 pb-4 pt-8">
        <p className="text-[1rem] font-semibold uppercase tracking-[0.08em] text-slate-500">
          Composite preview from components
        </p>
        <p className="text-[0.96rem] text-slate-900">
