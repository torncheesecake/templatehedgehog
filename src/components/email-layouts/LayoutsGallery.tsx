    ...
    <section className={VS.templates.library.frame}>
      <div className={VS.templates.library.heroGrid}>
        <header className={VS.templates.library.heroLead}>
          ...
          <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-[0.92rem] text-(--hedgehog-core-blue-deep)">
        ...
        <aside className={VS.templates.library.railCard}>
          ...
          <ul className="mt-3 divide-y divide-[rgba(222, 210, 204,0.14)]">
            {systems.map((system) => {
              ...
              return (
                <li key={system.slug} className="flex items-center justify-between gap-3 py-3">
                  <div>
                    <p className="text-[0.9rem] font-semibold text-(--hedgehog-core-navy)">{system.title}</p>
                    <p className="text-[0.82rem] leading-6 text-(--th-body-copy)">{system.description}</p>
                  </div>
                  <span className="inline-flex min-w-8 items-center justify-center rounded-full border border-[hsl(var(--th-accent)/0.28)] bg-[hsl(var(--th-accent)/0.1)] px-2.5 py-1 text-[0.78rem] font-semibold text-(--hedgehog-core-blue-deep)">
                    {count}
                  </span>
                </li>
              );
            ...
            <div className="mt-7 grid gap-x-10 gap-y-16 lg:grid-cols-2">
              {systemLayouts.map((layout) => (
                <Link
                  ...
                  className="group block rounded-[1rem] border border-[rgba(222, 210, 204,0.3)] bg-[#FDFDFD] p-5 shadow-[0_14px_28px_rgba(20, 19, 43,0.08)] transition duration-200 hover:-translate-y-[1px] hover:border-(--hedgehog-core-blue-deep) hover:shadow-[0_20px_34px_rgba(20, 19, 43,0.12)] focus:outline-none focus-visible:ring-2 focus-visible:ring-(--dune-focus) focus-visible:ring-offset-2"
                >
                  <LayoutPreviewStage
                    title={layout.title}
                  ...
                  <div className="mt-5">
                    <h3 className="text-[1.28rem] font-semibold leading-8 text-(--hedgehog-core-navy) group-hover:text-(--hedgehog-core-navy)">
                      {layout.title}
                    </h3>
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      ...
                      <span className="inline-flex rounded-full border border-[rgba(222, 210, 204,0.2)] bg-white px-2.5 py-1 text-[0.72rem] font-semibold tracking-[0.03em] text-(--hedgehog-core-navy)">
                        {layout.layoutSections.length} sections
                      </span>
                      <span className="inline-flex rounded-full border border-[rgba(222, 210, 204,0.32)] bg-[#fdfdfd] px-2.5 py-1 text-[0.72rem] font-semibold tracking-[0.03em] text-(--hedgehog-core-blue-deep)">
                        MJML + HTML
                      </span>
                    </div>
                    ...
                    <span className="mt-4 inline-flex items-center gap-1.5 text-[0.84rem] font-semibold text-(--hedgehog-core-blue-deep)">
                      Open layout
