...
function Cell({ enabled }: { enabled: boolean }) {
  return (
    <td className="py-3 text-center">
      <span
        className={`inline-block h-2.5 w-2.5 rounded-full ${enabled ? "bg-(--accent-primary)" : "bg-(--hedgehog-core-blue-deep)"}`}
...
function Header({ label }: { label: Role }) {
  return <th className="py-2 text-center text-sm font-medium text-(--dune-muted)">{label}</th>;
}
    ...
    <section className="rounded-2xl border border-(--hedgehog-core-blue-deep) bg-(--hedgehog-core-navy) p-5">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-(--dune-muted)">Visibility of role-based access across core actions</p>
        </div>
        <span className="inline-flex items-center gap-2 rounded-full border border-(--hedgehog-core-blue-deep) bg-(--hedgehog-core-navy) px-3 py-1 text-xs font-semibold text-(--accent-primary)">
      ...
      <div className="overflow-x-auto">
        <table className="w-full min-w-[640px]">
          <thead>
            <tr className="border-b border-(--hedgehog-core-blue-deep) text-left">
              <th className="pb-2 text-sm font-medium text-(--dune-muted)">Permission</th>
              <Header label="Admin" />
          ...
          <tbody>
            {matrix.map((row) => (
              <tr key={row.permission} className="border-b border-(--hedgehog-core-navy) last:border-0">
