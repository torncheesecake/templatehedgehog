...
export default async function LayoutDetailPage({ params }: Props) {
  ...
  return (
    <main className="min-h-screen text-(--th-body-copy)">
      ...
      <section className="relative isolate mx-auto w-full max-w-[1840px] overflow-hidden rounded-[1.3rem] border border-[rgba(222, 210, 204,0.34)] bg-(--surface-strong) px-5 pb-20 pt-10 shadow-[0_18px_36px_rgba(20, 19, 43,0.22)] sm:px-8 lg:px-14 lg:pb-24 lg:pt-12">
        <div className="pointer-events-none absolute inset-x-0 top-0 -z-20 h-[22rem] bg-[radial-gradient(circle_at_20%_20%,hsl(var(--th-accent)/0.26),transparent_50%),radial-gradient(circle_at_78%_16%,rgba(222, 210, 204,0.18),transparent_46%)]" />
        <article className="grid gap-10 pb-12 xl:grid-cols-[minmax(0,1.08fr)_minmax(360px,0.92fr)] xl:items-start">
          <div>
            ...
            <h1 className="mt-4 text-[2.35rem] font-semibold leading-[1.02] text-(--hedgehog-core-navy) sm:text-[3.2rem]">
              {layout.title}
            </h1>
            <p className="mt-4 max-w-[64ch] text-[1.14rem] leading-8 text-(--hedgehog-core-blue-deep)">
              {layout.description}
            </p>

            <div className="mt-5 flex flex-wrap items-center gap-x-4 gap-y-2 text-[1.03rem] text-(--hedgehog-core-blue-deep)">
            ...
            <div className="mt-7">
              {compiledHtml ? (
                ...
                <div className="rounded-[0.9rem] border border-[#d7c297] bg-[#fff5de] px-4 py-4 text-[1rem] leading-7 text-[#6b4a1b]">
          ...
          <div className="space-y-5">
            <article className="surface-card-soft relative overflow-hidden p-4 sm:p-5">
              <div className="pointer-events-none absolute inset-x-0 top-0 h-px bg-[linear-gradient(90deg,rgba(222, 210, 204,0.15),hsl(var(--th-accent)/0.42),rgba(222, 210, 204,0.15))]" />
              <p className="text-[1rem] font-semibold uppercase tracking-[0.09em] text-(--th-body-copy)">
                What this recipe covers
              </p>
              <h2 className="mt-2 text-[1.4rem] font-semibold text-(--hedgehog-core-navy)">Sections in sequence</h2>
              <ul className="mt-3 space-y-2.5 text-[0.98rem] leading-7 text-(--th-body-copy)">
                {layout.layoutSections.map((section) => (
                  <li key={`${layout.slug}-${section.title}`} className="flex items-start gap-3">
                    ...
                    <span>
                      <strong className="font-semibold text-(--hedgehog-core-navy)">{section.title}.</strong>{" "}
                      {section.description}
                    </span>
                  </li>
                ))}
            ...
            <article className="surface-card-soft relative overflow-hidden p-4 sm:p-5">
              ...
              <h2 className="mt-2 text-[1.4rem] font-semibold text-(--hedgehog-core-navy)">How developers usually use it</h2>
              <ul className="mt-3 space-y-2.5 text-[0.98rem] leading-7 text-(--th-body-copy)">
                <li className="flex items-start gap-3">
                  <Workflow className="mt-1 h-4.5 w-4.5 shrink-0 text-(--accent-primary)" />
            ...
            <article className="surface-card-soft relative overflow-hidden p-4 sm:p-5">
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <h2 className="mt-2 text-[1.54rem] font-semibold text-(--hedgehog-core-navy)">Component breakdown</h2>
                </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="mt-1 text-[1rem] uppercase tracking-[0.08em] text-(--hedgehog-core-blue-deep)">
        ...
        {missingComponentSlugs.length > 0 ? (
          <div className="mt-1 rounded-xl border border-[#c17f66] bg-[#ffece5] px-4 py-3 text-[1rem] leading-7 text-[#7a2f20]">
            Stack mismatch detected. Missing component definitions: {missingComponentSlugs.join(", ")}.
          </div>
        ) : null}

        <div className="mt-8 border-t border-[rgba(16,32,25,0.14)]" />
          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.82fr)] lg:items-center">
            <div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <Link
                  href="/docs"
                  className="inline-flex h-11 items-center rounded-[0.8rem] border border-[hsl(var(--th-accent)/0.52)] bg-[rgba(20,19,43,0.78)] px-5 text-[1rem] font-semibold tracking-[0.01em] !text-(--surface-strong) transition duration-200 hover:-translate-y-0.5 hover:border-(--dune-muted) hover:!text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--dune-focus) focus-visible:ring-offset-2"
                >
                  Read docs
                </Link>
              </div>
            ...
            <div className="surface-card-soft rounded-[0.96rem] bg-[rgba(20,19,43,0.46)] p-5">
              ...
              <ul className="mt-4 space-y-3 text-[1rem] leading-8 text-(--surface-strong)">
                <li className="border-b border-[rgba(222,210,204,0.22)] pb-2">See the full compiled email before editing</li>
                <li className="border-b border-[rgba(222,210,204,0.22)] pb-2">Understand which component blocks form the stack</li>
